...
*   **Feature Specifications:**
    ...
    *   **P2P Lending:** Fully functional peer-to-peer lending system connecting all accounts with: (1) User selection dropdown for loan requests showing all available users, (2) Real-time "Owed to Me" and "I Owe" summary sections with net balance calculation (excludes cancelled loans), (3) Overdue tracking with visual indicators and day counters, (4) Complete loan history displaying active, partially repaid, fully repaid, and cancelled statuses, (5) Bi-directional linking ensuring instant synchronization between lender and borrower accounts, (6) Flexible SQLAlchemy relationships for scalability, (7) Clear UI showing all loan details including due dates, repayment progress bars, circular progress indicators (with checkmark for fully repaid), status badges, and remaining balances, (8) Loan withdrawal/cancellation feature allowing lenders to cancel active loans with zero repayments, with automatic balance reversals and transaction recording for both parties. Includes 64 loan examples (4 per user) demonstrating all scenarios with balance validation and race condition protection.
