...
*   **Activity/Transactions Page Layout:** Responsive two-column layout for large screens (lg breakpoint) with calendar on the left (640px height, 3fr width) and right column (2fr width) containing vertically stacked statistics cards (Total Income, Total Expenses, Transactions) and All Transactions dropdown using flex-1 to fill remaining space and match calendar height. Mobile and tablet devices maintain traditional stacked layout with horizontal statistics cards followed by calendar and transactions list.
