...
*   **Feature Specifications:**
    ...
    *   **P2P Lending:** Comprehensive peer-to-peer lending system with loan creation, debt tracking, partial/full repayment support, and detailed transaction recording. Features balance validation, race condition protection with row-level locking, and automatic status updates. The system includes comprehensive test data with 64 loan examples (4 per user) demonstrating all loan scenarios: active loans with no repayments, partially repaid loans (both as lender and borrower), and fully repaid loans.
