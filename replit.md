...
*   **Activity/Transactions Page Layout:** Responsive two-column layout for large screens (lg breakpoint) with calendar and All Transactions dropdown in the left column (3fr width, stacked with space-y-4) and right column (2fr width) containing vertically centered statistics cards (Total Income, Total Expenses, Transactions count) using lg:justify-center and h-full for proper vertical alignment. Mobile and tablet devices maintain traditional stacked layout with horizontal statistics cards at top, followed by calendar, and transactions list below.
